...
|---|---|---|---|
| 1 | `layout.tsx` memakai `{ children }: { children: React.ReactNode }` | ~~Mempertahankan `LayoutProps<"/">`~~ → **dibatalkan, kembali ke rencana semula** | Koreksi ini **salah** dan dibuktikan salah oleh CI. `LayoutProps` di-generate Next.js ke `.next/types/**` yang gitignored, sedangkan CI menjalankan `typecheck` sebelum build apa pun — di checkout bersih tipe itu belum ada: `TS2304: Cannot find name 'LayoutProps'`. Lokal lulus hanya karena `.next/types` tersisa dari build sebelumnya, jadi hijau lokal adalah sinyal palsu. Pelajaran: jangan bergantung pada berkas ter-generate yang tidak ikut ter-commit |
